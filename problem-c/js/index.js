...
let uwOpponents = huskyGames2016.map(extractOpponent);
console.log(uwOpponents);
...
let opponentsUWLostTo = lostGames.map(game => extractOpponent(game));
console.log(opponentsUWLostTo);
...
lostGames.forEach(function(game){
  let opponent = extractOpponent(game);
  let uwScore = game.home === 'UW' ? game.home_score: game.opponent_score;
  let opponentScore = game.home === 'UW' ? game.opponent_score : game.home_score;
  console.log(opponent + ' at UW, ' + opponentScore + ' to ' + uwScore);
});
...
let gamesUWFumbles = huskyGames2016.filter(function(game){
  return game.fumbles > 0;
});
console.log(gamesUWFumbles.length);
...
function makeCombinedFilter(callback1, callback2){
  return function combinedFilter(game){
...
const fumbledAndLostFilter = makeCombinedFilter(huskiesLost, function(game){
  return game.fumbles > 0;
});
...
const gamesLostFumbles = huskyGames2016.filter(fumbledAndLostFilter);
console.log(gamesLostFumbles);
